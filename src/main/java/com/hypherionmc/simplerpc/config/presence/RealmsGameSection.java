 ...
 * @author HypherionSA
 *
 * Main RPC that is shown while the player is connected to a realm
 */
public final class RealmsGameSection implements RichPresenceContainer {

    @Path("enabled")
    @SpecComment("Enable/Disable the Realms Game Event")
    public boolean enabled = true;

    @Path("description")
    @SpecComment("The first line of text under the app name")
    public RandomArrayList<String>  description = RandomArrayList.of("Playing on {{realm.name}}");

    @Path("state")
    @SpecComment("The second line of text under the app name")
    public RandomArrayList<String>  state = RandomArrayList.of("Playing on a Realm");
    ...
    public RandomArrayList<String>  smallImageText = RandomArrayList.of("{{realm.description}}");
