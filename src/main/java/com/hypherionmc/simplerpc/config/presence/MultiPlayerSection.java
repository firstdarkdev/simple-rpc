 ...
 * @author HypherionSA
 *
 * Main RPC that is shown while the player is connected to a server
 */
public final class MultiPlayerSection implements RichPresenceContainer {
    ...
    public RandomArrayList<String>  description = RandomArrayList.of("Playing on {{server.name}} with {{server.players.count}} players");

    @Path("state")
    @SpecComment("The second line of text under the app name")
    public RandomArrayList<String>  state = RandomArrayList.of("Playing online");
    ...
    public RandomArrayList<String>  smallImageText = RandomArrayList.of();
