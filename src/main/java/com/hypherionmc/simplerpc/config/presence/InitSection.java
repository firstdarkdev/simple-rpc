 ...
 * @author HypherionSA
 *
 * Main RPC that is shown while the game is loading
 */
public final class InitSection implements RichPresenceContainer {

    @Path("enabled")
    @SpecComment("Enable/Disable the Game Loading Event")
    public boolean enabled = true;

    @Path("description")
    @SpecComment("The first line of text under the app name")
    public RandomArrayList<String> description = RandomArrayList.of("Minecraft is loading");

    @Path("state")
    @SpecComment("The second line of text under the app name")
    public RandomArrayList<String> state = RandomArrayList.of("Game Starting...");
